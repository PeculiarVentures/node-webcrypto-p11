    ...
    context("Sign/Verify", () => {
        ...
        keys.filter(key => key.usages.some(usage => usage === "sign"))
            .forEach(key => {
                it(key.name, done => {
                    // TODO: Add label
                    crypto.subtle.sign({ name: key.privateKey.algorithm.name }, key.privateKey, TEST_MESSAGE)
                        .then(sig => {
                            assert.equal(!!sig, true, "Has no signature value");
                            assert.notEqual(sig.length, 0, "Has empty signature value");
                            return crypto.subtle.verify({ name: key.publicKey.algorithm.name }, key.publicKey, sig, TEST_MESSAGE)
                        })
