...
import { Crypto, CryptoKey } from "../src";
...
function wrapContext(params: ITestParams, cb: Mocha.Func) {
  const test = params.skip
    ? it.skip
    : params.only
      ? context.only
      : context;
...
function isCryptoKeyPair(v: any): v is CryptoKeyPair {
  return "publicKey" in v && "privateKey" in v;
}
        ...
        context("Encrypt/Decrypt", () => {
          param.actions.encrypt!.forEach((action, index) => {
            wrapTest(async () => {
              ...
              assert.equal(Convert.ToUtf8String(dec), Convert.ToUtf8String(action.data));
            }, action, index);
          });
        });
      }
